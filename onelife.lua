    ...
    if not storage.SM_Store.oneLifeMode then
        if player.controller_type == defines.controllers.spectator then
            player.set_controller {
                ...
                character = game.surfaces[1].create_entity({name = "character", position = game.surfaces[1].find_non_colliding_position("character", {x = 0, y = 0}, 1024, 0.1, false), force = game.forces.player})
            }
